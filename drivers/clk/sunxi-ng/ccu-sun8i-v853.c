...
static const struct clk_hw *pll_periph_160M_hws[] = { &pll_periph_160M_clk.hw };
...
static CLK_FIXED_FACTOR_HW(pll_video_2x_clk, "pll-video-2x",
			    &pll_video_4x_clk.common.hw, 2, 1, CLK_SET_RATE_PARENT);
static CLK_FIXED_FACTOR_HW(pll_video_clk, "pll-video",
			    &pll_video_4x_clk.common.hw, 4, 1, CLK_SET_RATE_PARENT);


/*
 * For the CSI PLL, the output divider is described as "only for testing"
 * in the user manual. So it's not modelled and forced to 0.
 */
...
static CLK_FIXED_FACTOR_HW(pll_csi_2x_clk, "pll-csi-2x",
			    &pll_csi_4x_clk.common.hw, 2, 1, CLK_SET_RATE_PARENT);
...
static struct ccu_nm pll_audio_base_clk = {
	...
	.common		= {
		...
		.hw.init	= CLK_HW_INIT_PARENTS_DATA("pll-audio-base", osc24M,
							   &ccu_nm_ops,
							   CLK_SET_RATE_UNGATE),
	},
};

static const struct clk_hw *pll_audio_base_hws[] = {
	&pll_audio_base_clk.common.hw
};
static SUNXI_CCU_M_HWS(pll_audio_div2_clk, "pll-audio-div2",
		       pll_audio_base_hws, 0x078, 16, 3, 0);
static SUNXI_CCU_M_HWS(pll_audio_div5_clk, "pll-audio-div5",
		       pll_audio_base_hws, 0x078, 20, 3, 0);

static const struct clk_hw *pll_audio_div2_hws[] = {
	&pll_audio_div2_clk.common.hw
};
static SUNXI_CCU_M_HWS(pll_audio_4x_clk, "pll-audio-4x",
		       pll_audio_div2_hws, 0xE00, 5, 5, 0);
...
static SUNXI_CCU_M_HWS(pll_audio_1x_clk, "pll-audio-1x",
		       pll_audio_div5_hws, 0xE00, 0, 5, 0);
...
static CLK_FIXED_FACTOR_HW(pll_npu_clk, "pll-npu",
			    &pll_npu_4x_clk.common.hw, 4, 1, CLK_SET_RATE_PARENT);
...
static SUNXI_CCU_M_HWS(cpu_axi_clk, "cpu-axi",
		       cpu_hws, 0x500, 0, 2, 0);
static SUNXI_CCU_M_HWS(cpu_apb_clk, "cpu-apb",
		       cpu_hws, 0x500, 8, 2, 0);

static const struct clk_parent_data ahb_apb0_apb1_parents[] = {
	{ .fw_name = "hosc" },
	{ .fw_name = "losc" },
	{ .fw_name = "iosc" },
	{ .hw = &pll_periph_clk.hw },
};
static SUNXI_CCU_MP_DATA_WITH_MUX(ahb_clk, "ahb", ahb_apb0_apb1_parents, 0x510,
				  0, 5,		/* M */
				  8, 2,		/* P */
				  24, 2,	/* mux */
				  0);

static SUNXI_CCU_MP_DATA_WITH_MUX(apb0_clk, "apb0", ahb_apb0_apb1_parents, 0x520,
				  0, 5,		/* M */
				  8, 2,		/* P */
				  24, 2,	/* mux */
				  0);

static SUNXI_CCU_MP_DATA_WITH_MUX(apb1_clk, "apb1", ahb_apb0_apb1_parents, 0x524,
				  0, 5,		/* M */
				  8, 2,		/* P */
				  24, 2,	/* mux */
				  0);
...
static const struct clk_hw *i2s_dmic_codec_parents[] = {
	&pll_audio_1x_clk.common.hw,
	&pll_audio_4x_clk.common.hw,
};
static SUNXI_CCU_M_HW_WITH_MUX_GATE(i2s1_clk, "i2s1", i2s_dmic_codec_parents, 0xa14,
				     0, 4,	/* M */
				     24, 1,	/* mux */
				     BIT(31),	/* gate */
				     0);
...
static SUNXI_CCU_M_HW_WITH_MUX_GATE(dmic_clk, "dmic", i2s_dmic_codec_parents, 0xa40,
				     0, 4,	/* M */
				     24, 1,	/* mux */
				     BIT(31),	/* gate */
				     0);
...
static SUNXI_CCU_M_HW_WITH_MUX_GATE(audio_dac_clk, "audio-dac", i2s_dmic_codec_parents, 0xa50,
				     0, 4,	/* M */
				     24, 1,	/* mux */
				     BIT(31),	/* gate */
				     0);

static SUNXI_CCU_M_HW_WITH_MUX_GATE(audio_adc_clk, "audio-adc", i2s_dmic_codec_parents, 0xa54,
				     0, 4,	/* M */
				     24, 1,	/* mux */
				     BIT(31),	/* gate */
				     0);
...
static struct ccu_mux usb_ohci_clk = {
	...
	.common		= {
		...
		.hw.init	= CLK_HW_INIT_PARENTS_DATA("usb-ohci",
							   usb_ohci_parents,
							   &ccu_mux_ops,
							   0),
	},
...
static SUNXI_CCU_GATE_HWS(bus_riscv_cfg_clk, "bus-riscv-cfg", ahb_hws,
			  0xd0c, BIT(0), 0);
...
static struct ccu_reset_map sun8i_v853_ccu_resets[] = {
	...
	[RST_RISCV]			= { 0xd04, BIT(1), CCU_FEATURE_KEY_FIELD },
...
static int sun8i_v853_ccu_probe(struct platform_device *pdev)
{
	 ...
	 *
	 * See the comment before pll-video definition for the reason.
	 */
